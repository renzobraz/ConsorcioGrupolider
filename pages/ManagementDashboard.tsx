  ...
  useEffect(() => {
    const calculateManagementData = async () => {
      ...
      try {
        ...
        try {
          // Solicitando apenas o que é essencial para o dashboard gerencial
          await db.getAllCreditUsages();
          allPayments = await db.getAllPaymentsDictionary();
        } catch (dbErr) {
          console.warn("Falha ao carregar dados do Supabase para o dashboard gerencial, seguindo com lista vazia.");
        }
        ...
        const quotasToProcess = quotas;

        quotasToProcess.forEach(quota => {
          ...
          if (globalFilters.status === 'ACTIVE') matchStatus = !quota.isContemplated;
          if (globalFilters.status === 'CONTEMPLATED') matchStatus = quota.isContemplated;

          if (!matchCompany || !matchAdmin || !matchProduct || !matchQuota || !matchStatus) return;

          const schedule = generateSchedule(quota, indices);
          const paymentMap = paymentMapByQuota(paymentMapByQuotaList(allPayments, quota.id));
        ...
        // Helper function to safely extract payments for quota
        function paymentMapByQuotaList(allPaymentsMap: Record<string, any>, qId: string) {
          return allPaymentsMap[qId] || {};
        }
        function paymentMapByQuota(qPayments: any) {
          return qPayments;
        }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2 animate-in slide-in-from-top duration-300">
            <Activity className="text-emerald-600" id="activity-icon" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in duration-300">
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-sm text-slate-500 font-medium font-sans">Total Desembolsado</p>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-sm text-slate-500 font-medium font-sans">Patrimônio (Fundo Comum)</p>
          <h3 className="text-2xl font-bold text-slate-900 mt-1">{formatCurrency(realTotals.totalFC)}</h3>
          <div className="mt-3 flex items-center gap-1 text-xs text-emerald-600">
            <ArrowUpRight size={12} />
            <span>{formatPercent((realTotals.totalFC / (realTotals.totalPaid || 1)) * 100)} do total</span>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-sm text-slate-500 font-medium font-sans">Custo (Taxas/Seguros)</p>
          <h3 className="text-2xl font-bold text-slate-900 mt-1">{formatCurrency(realTotals.totalFees)}</h3>
          <div className="mt-3 flex items-center gap-1 text-xs text-amber-600">
             <span>{formatPercent((realTotals.totalFees / (realTotals.totalPaid || 1)) * 100)} do total</span>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-sm text-slate-500 font-medium font-sans">Encargos (Multas/Juros)</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-slate-800 flex items-center gap-2">
              <PieChart size={18} className="text-emerald-600" id="pie-chart-header" />
            ...
            <div className="w-full md:w-1/2 space-y-4">
              {compositionData.map((item, index) => (
                <div key={item.name} className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold text-slate-900">{formatPercent((item.value / (realTotals.totalPaid || 1)) * 100)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="space-y-4">
            {adminPerformance.map((admin) => (
              <div key={admin.name} className="space-y-2 animate-in fade-in duration-300">
                ...
                <div className="w-full bg-slate-100 rounded-full h-2">
                  <div 
                    ...
                    style={{ width: `${adminPerformance[0]?.total ? (admin.total / adminPerformance[0].total) * 100 : 0}%` }}
                  ></div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-emerald-950 text-white p-6 rounded-xl shadow-lg flex flex-col justify-between relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10">
            <TrendingUp size={120} />
          </div>
          
          <div>
            <h3 className="text-emerald-300 font-medium mb-1 font-sans">Custo Médio por Parcela</h3>
            <p className="text-3xl font-bold">{formatCurrency(realTotals.avgInstallment)}</p>
          </div>

          <div className="mt-8 space-y-4">
            <div className="p-4 bg-white/10 rounded-lg backdrop-blur-sm">
              <p className="text-emerald-300 text-xs uppercase tracking-wider font-bold mb-1 font-mono">Eficiência Financeira</p>
