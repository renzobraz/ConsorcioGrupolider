...
export const db = {
  ...
  getUserByEmail: async (email: string): Promise<User | null> => {
    ...
    if (supabase) {
      try {
        ...
        console.warn(`Error fetching user by email ${email}:`, err.message);
        return null;
      }
    } else {
  ...
  getUserById: async (id: string): Promise<User | null> => {
    ...
    if (supabase) {
      try {
        ...
        console.warn(`Error fetching user by id ${id}:`, err.message);
        return null;
      }
    } else {
      const users = JSON.parse(localStorage.getItem(USERS_KEY) || '[]');
  ...
  saveUser: async (user: User): Promise<void> => {
    ...
    if (supabase) {
      ...
      // Fallback: Se o banco reclamar que a coluna company_id não existe, tenta salvar sem ela
      if (error && (error.message.includes('column "company_id"') || error.code === 'PGRST204')) {
        console.warn("Schema mismatch: company_id column not found. Retrying without it.");
        const { company_id, ...legacyPayload } = payload;
        const { error: retryError } = await supabase.from('users').upsert(legacyPayload);
